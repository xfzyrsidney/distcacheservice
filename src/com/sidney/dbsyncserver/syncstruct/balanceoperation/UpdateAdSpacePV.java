...
public class UpdateAdSpacePV implements IOracleOperator
{
	private static Log logger = LogFactory.getLog(UpdateAdSpacePV.class);
	private Object lockObj = new Object();
	private static UpdateAdSpacePV thisInstance = new UpdateAdSpacePV();

	public static  UpdateAdSpacePV getInstance()
	{
		if (thisInstance == null)
		{
			synchronized (UpdateAdSpacePV.class)
			{
				try
				{
					if (null == thisInstance)
					{
						thisInstance = new UpdateAdSpacePV();
					}
				} catch (Exception e)
				{
					thisInstance = null;
		...
		try
		{
			synchronized (lockObj)
			{
				...
				if (lstReturnFailed != null && lstReturnFailed.size() > 0)
				{
					...
					String tempLog = String.format("[UpdateAdSpacePV]:(size:%d) %s", lstReturnFailed.size(), sb);
